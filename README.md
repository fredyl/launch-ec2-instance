...
def replace_null_values(data_key, t_cols):
    if isinstance(data_key, dict):
        return {k: replace_null_values(v, t_cols) if v is not None else "" for k, v in data_key.items()}
    elif isinstance(data_key, list):
        return [replace_null_values(item, t_cols) if item is not None else "" for item in data_key]
    else:
...
def get_holman_data(token, data_type, data_key,url_ext=None):
    '''
    Get the data from holman api and return the data in a list
    '''
...
t_cols= ["orderReceivedDate","orderPlacedDate","scheduledProductionDate","deliveryPaperMailed","shipDate","deliveredToDealerDate","notifiedOfDelivery","upfitInvoiceDate"]
...
for data_type, (data_key, primary_key) in holman_endpoints.items():
    ...
    if data_type == 'vehicles':
        data_list = replace_null_values(data_list, t_cols)
    else:
        data_list
    # data_list = replace_null_values(data_list)
    if data_list:
        Holman_Upsert_data(data_type, data_list, primary_key, merge_keys=None)
    else:
...
print(f"Data upserted for data type {data_type}")
